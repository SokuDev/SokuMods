...
std::shared_ptr<CompiledString> CompiledStringFactory::compileString(const char *str)
{
	...
	if (var != str)
		return std::make_shared<ComplexCompiledString>(
			std::make_shared<BasicCompiledString>(std::string(str, var)),
			CompiledStringFactory::compileString(var)
		);
...
std::shared_ptr<CompiledString> CompiledStringFactory::_compileVar(const char *str)
{
	...
	try {
		auto compiled = std::shared_ptr<CompiledString>(
			CompiledStringFactory::_varsMakers.at({str, end})()
		);

		if (end[2])
			return std::make_shared<ComplexCompiledString>(
				compiled,
				CompiledStringFactory::compileString(end + 2)
			);
		return compiled;
