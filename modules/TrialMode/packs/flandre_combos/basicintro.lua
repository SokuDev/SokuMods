...
local stop = false
local playerInfo = PlayerInfo.new(enums.characters.utsuho, 0, false)
local okuu
...
local function init()
	...
	okuu = Character.new(playerInfo)
	okuu.action = enums.actions.ACTION_USING_SC_ID_212
	okuu:initAnimation()
	okuu.position.x = battleMgr.leftChr.position.x + (battleMgr.rightChr.position.x - battleMgr.leftChr.position.x) / 2
end
...
function update()
	if okuu then
		okuu:updateMove()
		if okuu.action == 0 then
			if alpha ~= 0 then
				alpha = alpha - 15
				okuu.renderInfos.color.a = alpha
			end
		end
		okuu.objects:update()
	end
