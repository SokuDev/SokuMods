...
void inputBoxUpdate()
{
	...
	if (timers[VK_DELETE] == 1 || (timers[VK_DELETE] > 36 && timers[VK_DELETE] % 6 == 0)) {
		if (cursorPos < buffer.size() - 1) {
			...
			SokuLib::playSEWaveBuffer(0x27);
			changes = true;
			textSprite.texture.createFromText(sanitizeInput().c_str(), font, {8 * buffer.size(), 1800});
		}
	}
	...
	if (changes)
		textSprite.texture.createFromText(sanitizeInput().c_str(), font, {max(292, 8 * buffer.size()), 1800});
	changes = false;
	mutex.unlock();
}
...
void openInputDialog(const char *title, const char *defaultValue)
{
	...
	titleSprite.texture.createFromText(title, font, {292, 32});
	textSprite.texture.createFromText(sanitizeInput().c_str(), font, {max(292, 8 * buffer.size()), 20});
